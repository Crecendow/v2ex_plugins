...
import { fetchHotPosts, fetchLatestPosts, fetchPostDetail, fetchReplies } from '../api/v2ex';
...
function getThemeColors(): ThemeColors {
  ...
  const getColor = (key: string, defaultValue: string): string => {
    const color = config.get<string>(`workbench.colorCustomizations.${key}`);
    if (color) {
      return color;
    }
...
export class V2EXPostTreeDataProvider implements vscode.TreeDataProvider<PostTreeItem> {
  private _onDidChangeTreeData: vscode.EventEmitter<PostTreeItem | undefined> = new vscode.EventEmitter<PostTreeItem | undefined>();
  readonly onDidChangeTreeData: vscode.Event<PostTreeItem | undefined> = this._onDidChangeTreeData.event;

  private posts: V2EXPost[] = [];
  ...
  async openPostDetail(postId: number): Promise<void> {
  ...
  private createWebviewPanel(post: V2EXPost, replies: V2EXReply[], colors: ThemeColors): void {
  ...
  private getWebviewContent(post: V2EXPost, replies: V2EXReply[], colors: ThemeColors): string {
...
export class PostTreeItem extends vscode.TreeItem {
  constructor(public readonly post: V2EXPost) {
    super(post.title);
